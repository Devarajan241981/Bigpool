...
export default function ProductDetailClient({ id, serverProduct }: Props) {
  const router = useRouter();
  const { products } = useProductStore();
  const storeProduct = products.find((p) => p.id === id);
  const [fetchedProduct, setFetchedProduct] = useState<typeof storeProduct>(serverProduct ?? undefined);
  const [productLoading, setProductLoading] = useState(!storeProduct && !serverProduct);
